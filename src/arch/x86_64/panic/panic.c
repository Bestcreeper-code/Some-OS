...
void _Log_Isr_Error_Code(char isr_idx, uint64_t code){
...
void _panic_handler(uint64_t isr_index, uint64_t err_code, cpu_registers_t* regs, uint64_t* call_stack) {
    ...
    Sys_log("Kernel panic (%lx | %lx | CR2:0x%lx)\n",
        (uint64_t)isr_index,
        (uint64_t)err_code,
        (uint64_t)regs->cr2);
    ...
    Sys_log_NoPos("=======================================================================\n");
    Sys_log_NoPos("KERNEL PANIK -> ISR Index: %lx (%s), Error Code: %lx\n",
    ...
    if (call_stack) {
        ...
        for (int i = 0; i < MAX_STACK_TRACE_SIZE; i++) {
            ...
            Sys_color_log_NoPos("    %s (%p)\n",
                ANSI_BRIGHT_YELLOW, ANSI_BG_BLACK, sym->str, (void*)addr);
        }
    }
